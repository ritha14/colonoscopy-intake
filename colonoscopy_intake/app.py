...
def step_6():
    ...
    if has_insurance:
        ...
        insurance_type_key = opt_keys[sel_idx]

        insurance_carrier = st.text_input(
            "Insurance carrier / plan name *",
            value=d.get("insurance_carrier", ""),
            placeholder="e.g., Blue Cross Blue Shield, Aetna, Humana, Medicare…",
        )
        ...
        ins_front = st.file_uploader("Insurance Card — FRONT *", type=["jpg", "jpeg", "png", "pdf", "heic"], key="uf_ins_front")
        if ins_front:
        ...
        ins_back = st.file_uploader("Insurance Card — BACK *", type=["jpg", "jpeg", "png", "pdf", "heic"], key="uf_ins_back")
        if ins_back:
    ...
    dl = st.file_uploader("Driver's License / ID *", type=["jpg", "jpeg", "png", "pdf", "heic"], key="uf_dl")
    if dl:
    ...
    with cn2:
        if st.button("Next →", type="primary", key="s6_next"):
            errors = []
            if has_insurance:
                if not insurance_carrier.strip():
                    errors.append("Please enter your insurance carrier or plan name.")
            ...
            ins_label = INSURANCE_TYPES.get(insurance_type_key, {}).get("label", "Self-Pay") if has_insurance else "Self-Pay"
            st.session_state.data.update({
                "has_insurance": has_insurance,
                "insurance_type": insurance_type_key if has_insurance else "self_pay",
                "insurance_type_label": ins_label,
                "insurance_carrier": insurance_carrier.strip() if has_insurance else "N/A",
            ...
            st.session_state.ins_result = None  # force re-analysis if returning
            _go(7)
            _rerun()


# ── Step 7: Insurance Eligibility ────────────────────────────────────────────
def step_7():
    ...
    if st.session_state.ins_result is None:
        with st.spinner("Reviewing your insurance…"):
            result = analyze_insurance(d.get("insurance_type", "self_pay"))
    ...
    if result_code == "ELIGIBLE":
        _box("ok", f"<strong>✅ Eligible for Full Coverage</strong><br><br>{r['message']}")

    elif result_code in ("CASH_PAY_SURGEON", "CASH_PAY_FULL"):
        _box("warn", f"<strong>💳 Cash Pay Required</strong><br><br>{r['message']}")

    elif result_code == "NOT_ELIGIBLE":
        _box("err", f"<strong>❌ Not Eligible</strong><br><br>{r['message']}")
        st.info(f"If you have questions, please call our office at {OFFICE_PHONE}.")

        col_a, col_b = st.columns(2)
        with col_a:
            if st.button("← Update My Insurance Info", key="s7_back_ineligible"):
                st.session_state.ins_result = None
                _go(6)
                _rerun()
        with col_b:
            if st.button("Submit & Exit", key="s7_end_ineligible"):
        ...
        _box("info", f"<strong>📋 Office Verification Required</strong><br><br>{r['message']}")
    ...
    with cn1:
        if st.button("← Back", key="s7_back"):
            ...
            _rerun()
    with cn2:
        if st.button("Next →", type="primary", key="s7_next"):
            _go(8)
            _rerun()
